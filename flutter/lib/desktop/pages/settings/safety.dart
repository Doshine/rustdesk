...
class SafetySettings extends StatefulWidget {
  const SafetySettings({Key? key}) : super(key: key);

  @override
  State<SafetySettings> createState() => _SafetyState();
}

class _SafetyState extends State<SafetySettings> with AutomaticKeepAliveClientMixin {
  @override
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
        ...
        child: Column(
          children: [
            settingsLock(locked, 'Unlock Security Settings', () {
              locked = false;
              setState(() => {});
            }),
            preventMouseKeyBuilder(
              ...
              child: Column(children: [
                permissions(context),
                password(context),
                SettingsCard(title: '2FA', children: [tfa()]),
                if (!isChangeIdDisabled())
                  SettingsCard(title: 'ID', children: [changeId()]),
                more(context),
              ]),
            ),
          ],
  ...
  Widget tfa() {
    ...
    tmpWrapper() {
      ...
      ).marginOnly(left: kSettingsCheckBoxLeftMargin + 30);
        ...
        children: [
          Flexible(
            child: Tooltip(
              ...
              child: SettingsOptionCheckBox(context, "Enable trusted devices",
  ...
  Widget changeId() {
    return ChangeNotifierProvider.value(
        ...
        child: Consumer<ServerModel>(builder: ((context, model, child) {
          return SettingsButton('Change ID', changeIdDialog,
  ...
  Widget permissions(context) {
    ...
    tmpWrapper() {
      ...
      return SettingsCard(title: 'Permissions', children: [
        ComboBox(
            keys: [
          ...
          children: [
            ...
            if (isWindows)
              SettingsOptionCheckBox(
                  ...
                  enabled: enabled, fakeValue: fakeValue),
            SettingsOptionCheckBox(context, 'Enable clipboard', kOptionEnableClipboard,
                enabled: enabled, fakeValue: fakeValue),
            SettingsOptionCheckBox(
                context, 'Enable file transfer', kOptionEnableFileTransfer,
                enabled: enabled, fakeValue: fakeValue),
            SettingsOptionCheckBox(context, 'Enable audio', kOptionEnableAudio,
                enabled: enabled, fakeValue: fakeValue),
            SettingsOptionCheckBox(context, 'Enable camera', kOptionEnableCamera,
                enabled: enabled, fakeValue: fakeValue),
            SettingsOptionCheckBox(context, 'Enable terminal', kOptionEnableTerminal,
                enabled: enabled, fakeValue: fakeValue),
            SettingsOptionCheckBox(
            ...
            SettingsOptionCheckBox(
                ...
                enabled: enabled, fakeValue: fakeValue),
            if (isWindows)
              SettingsOptionCheckBox(context, 'Enable blocking user input',
            ...
            if (bind.mainSupportedPrivacyModeImpls() != '[]')
              SettingsOptionCheckBox(
                  ...
                  enabled: enabled, fakeValue: fakeValue),
            SettingsOptionCheckBox(context, 'Enable remote configuration modification',
    ...
    return ChangeNotifierProvider.value(
        ...
        child: Consumer<ServerModel>(builder: ((context, model, child) {
          ...
          List<Widget> radios = passwordValues
              .map((value) => SettingsRadio<String>(
                    context,
                    value: value,
          ...
          final numericOneTimePassword = GestureDetector(
            ...
            onTap: isNumOPTChangable
                ? () => model.switchAllowNumericOneTimePassword()
                : null,
          ).marginOnly(left: kSettingsContentHSubMargin - 5);
          ...
          return SettingsCard(title: 'Password', children: [
            ComboBox(
            ...
            if (usePassword && !isChangePermanentPasswordDisabled())
              SettingsSubButton('Set permanent password', setPasswordDialog,
                  permEnabled && !locked),
            // if (usePassword)
            //   hide_cm(!locked).marginOnly(left: kSettingsContentHSubMargin - 6),
  ...
  Widget more(BuildContext context) {
    bool enabled = !locked;
    return SettingsCard(title: 'Security', children: [
      shareRdp(context, enabled),
      SettingsOptionCheckBox(context, 'Deny LAN discovery', 'enable-lan-discovery',
          reverse: true, enabled: enabled),
      ...directIp(context),
      whitelist(),
      ...autoDisconnect(context),
      SettingsOptionCheckBox(context, 'keep-awake-during-incoming-sessions-label',
          kOptionKeepAwakeDuringIncomingSessions,
          reverse: false, enabled: enabled),
      if (bind.mainIsInstalled())
        SettingsOptionCheckBox(context, 'allow-only-conn-window-open-tip',
  ...
  List<Widget> directIp(BuildContext context) {
    ...
    return [
      SettingsOptionCheckBox(context, 'Enable direct IP access', kOptionDirectServer,
      ...
      () {
        ...
        tmpWrapper() {
          ...
          return Offstage(
            offstage: !enabled,
            child: SettingsSubLabeledWidget(
              context,
              'Port',
              Row(children: [
                SizedBox(
